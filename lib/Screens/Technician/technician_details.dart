...
class TechnicianDetailScreen extends StatefulWidget {
  ...
  const TechnicianDetailScreen({super.key, required this.techData, required this.docId});
...
class _TechnicianDetailScreenState extends State<TechnicianDetailScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    String name = widget.techData['name'] ?? "Technician Name";
    String category = widget.techData['category'] ?? "General";
    String address = widget.techData['address'] ?? "Dhaka, Bangladesh";
    String experience = widget.techData['experience'] ?? "0 Years";
    bool isVerified = widget.techData['status'] == 'approved';

    double totalRatingSum = (widget.techData['totalRatingSum'] ?? 0.0).toDouble();
    int ratingCount = (widget.techData['ratingCount'] ?? 0).toInt();
    double averageRating = ratingCount > 0 ? (totalRatingSum / ratingCount) : 0.0;

    Uint8List? imageBytes;
    if (widget.techData['imageBase64'] != null && widget.techData['imageBase64'].toString().isNotEmpty) {
      imageBytes = base64Decode(widget.techData['imageBase64']);
    }
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 20),
                  // Experience Show korchi Bio er bodole
                  const Text("Experience", style: TextStyle(color: Color(0xFFFFC65C), fontSize: 16, fontWeight: FontWeight.bold)),
                  const SizedBox(height: 8),
                  Text(experience, style: const TextStyle(color: Colors.white54, fontSize: 14, height: 1.5)),
                ],
              ),
            ),
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      Expanded(child: _buildFormField("Date", Icons.calendar_today_outlined, "Select Date", _dateController)),
                      const SizedBox(width: 15),
                      Expanded(child: _buildFormField("Time", Icons.access_time_outlined, "09:00 AM", _timeController)),
                    ],
                  ),
                  const SizedBox(height: 15),
                      ...
                      onPressed: () async {
                        ...
                        try {
                          await FirebaseFirestore.instance.collection('bookings').add({
                            ...
                            'techImageBase64': widget.techData['imageBase64'],
